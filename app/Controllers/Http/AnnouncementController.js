...
class AnnouncementController {
    ...
    async store({ request, auth, response, session }) {
        try {
            ...
            if (format.test(name_announcement)) {
                session.flash({ AnnouncementAddError: 'Contient des caractères spéciaux' });
    ...
    */
    ...
    async update({ auth, params, session, response, request }) {
        try {
            ...
            const description = request.input('description')
            if (format.test(name_announcement)) {
                session.flash({ editAnnouncementError: 'Votre annonce contient des caractères speciaux' });
            ...
            if (announcement) {
                if (announcement.user_id == user.id || user.admin == 1) {
                    ...
                    announcement.description = description;
                    announcement.save();
                    session.flash({ editAnnouncementSuccess: 'Modifiée avec succès' });
                    return response.redirect('/announcement/' + params.id + '/edit')
                } else {
                    throw 'error'
                }
